class Solution {
public:
    string findLine(vector<string>& words, int maxWidth, int i, int j, int eachSpaceSlotLength, int extraSpaceSlotLength){
        string line;
        for(int k = i;k < j;k++){
            line += words[k];
            if(k == j-1){
    ...
    vector<string> fullJustify(vector<string>& words, int maxWidth) {
        ...
        while(i < n){
            ...
            int spaceSlot = 0;
            int j = i+1;
            while(j < n && words[j].size() + 1 + lettersCount + spaceSlot <= maxWidth){
            ...
            string line = findLine(words, maxWidth, i, j, eachSpaceSlotLength, extraSpaceSlotLength);
            i = j;
